...
void TCP::decode(MemoryStream &stream)
{
    ...
    ecn = stream.read<int>(1);
    cwr = stream.read<int>(1);
    ece = stream.read<int>(1);
    urg = stream.read<int>(1);
    ack = stream.read<int>(1);
    psh = stream.read<int>(1);
    rst = stream.read<int>(1);
    syn = stream.read<int>(1);
    fin = stream.read<int>(1);
